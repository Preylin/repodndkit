...
import { makeColumns, makeData, type Person } from './makeDataColumn'
...
export function ColumnVirtualTable() {
  const columns = React.useMemo<ColumnDef<Person>[]>(
    () => makeColumns(20) as ColumnDef<Person>[], // ◄ Un pequeño casteo aquí asegura la compatibilidad inmediata
    []
  );

  const [data, setData] = React.useState<Person[]>(() => makeData(1000, columns));

  const refreshData = React.useCallback(() => {
    setData(makeData(1000, columns)); // ◄ ¡Adiós error de asignación!
  }, [columns]);
