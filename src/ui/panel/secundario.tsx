...
export function ShowMain1() {
  ...
  return (
    <div className="flex flex-col w-full h-dvh overflow-hidden">
      ...
      <main className="relative flex flex-row w-full h-full overflow-hidden bg-gray-50">
        ...
        {/* CUERPO PRINCIPAL */}
        <section className="flex-1 px-4 py-2 overflow-auto relative bg-amber-300">
          {/* BOTÓN: Solo se muestra aquí si el sidebar NO está abierto */}
          {!isOpen && (
            <button
              onClick={() => setIsOpen(true)}
              className="absolute top-1 left-1 bg-indigo-600 text-white px-4 py-2 rounded-lg shadow-lg z-20 hover:bg-indigo-700 transition-all"
            >
              Abrir
            </button>
          )}
  ...
  return (
    <nav className="p-4">
      <div className="flex flex-row items-center justify-between mb-6 px-2">
        ...
        {/* BOTÓN: Solo se muestra aquí si el sidebar ESTÁ abierto */}
        {isOpen && (
          <button 
            onClick={toggleMenu}
